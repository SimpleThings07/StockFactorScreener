...
def fetch_historical_net_income (stock):
    ...
    try:
        ...
        logging.debug("Quarterly Latest Net Income (Ticker: %s): %:,.2f USD", stock.ticker, latest_quarter)
        logging.debug("Quarterly Same Previous Year Net Income (Ticker: %s): %,.2f USD", stock.ticker, previous_year_quarter)
...
def save_to_excel (stock_data, config):
    ...
    for ticker_data in tickers_and_weights:
        ticker = ticker_data["ticker"] if isinstance(ticker_data, dict) else ticker_data[0]
        if ticker in stock_data:
            ordered_stock_data[ticker] = stock_data[ticker]
    ...
    ws['A1'] = "ETF"
    ws['B1'] = "MSCI World Momentum Index"
    ws['A2'] = "Total Investment"
    ws['B2'] = "1.000,00 €"

    # Add hyperlink to "S&P SmallCap Quality"
    ws['B1'].hyperlink = "https://www.ishares.com/uk/individual/en/products/270051/ishares-msci-world-momentum-factor-ucits-etf?switchLocale=y&siteEntryPassthrough=true"
...
def analyze_stock (config, stock, ticker, weight):
    ...
    try:
        ...
        roe_ttm_yf = get_roe (stock.info, ticker)
        ...
        roa_ttm_yf = get_roa (stock.info, ticker)
...
def analyze_one_stock (ticker_data):
...
if __name__ == "__main__":
    ...
    try:
        ...
        with ThreadPoolExecutor (max_workers=MAX_WORKERS) as executor:
            ...
            futures = [ executor.submit ( analyze_one_stock, ticker_data ) for ticker_data in tickers_and_weights ]
